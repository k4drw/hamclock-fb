...
```bash
curl -sSL https://raw.githubusercontent.com/k4drw/hamclock-fb/master/install/install.sh | sudo bash
```
...
1. Create the update script:
   ```bash
   sudo cp hamclock-update /usr/local/sbin/
   sudo chmod +x /usr/local/sbin/hamclock-update
   ```
...
3. Install the systemd services:
   ...
   sudo cp hamclock-update.service /etc/systemd/system/
   sudo cp hamclock-update.timer /etc/systemd/system/
   ```
